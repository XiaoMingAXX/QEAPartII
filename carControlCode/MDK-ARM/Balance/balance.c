...
//				
				...
				if(last_mode != Mode)  //每次切换模式需要刷新屏幕
				{
				 ...
				 if(Mode == APP_Control_Mode)      	Qianpo(); //   Get_RC();             //Handle the APP remote commands //处理APP遥控命令    
			  else if(Mode == PS2_Control_Mode) jingti();  //PS2_Control();        //Handle PS2 controller commands //处理PS2手柄控制命令
			  else if(Mode == Lidar_Avoid_Mode)  youyu();  // Lidar_Avoid();        //Avoid Mode //避障模式
			  else if(Mode == Lidar_Follow_Mode)   pianzhi();// Lidar_Follow();       //Follow Mode //跟随模式
	...
	for(int j=0;j<1152;j++)
	  {
			if(Dataprocess[j].angle>268 && Dataprocess[j].angle<272)   //取雷达的4度的点
			{
				if(i==0)
				{
				 ...
				 if(Dataprocess[j].distance<(target_distance+100))//限制一下雷达的探测距离
				 {
					 data_count++;
